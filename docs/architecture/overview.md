...
- Builds: workspace-scoped production runs of a `DesignRevision` for a target quantity (see ADR 0021). A `Build` freezes the revision's BOM into `BuildLineItem` snapshots (one per BOM line, carrying the chosen part + source location) and `BuildDesignatorAssignment` rows (one per reference designator). The build advances through `created → allocated → started → in_progress → completed`, plus `cancelled`. Stock side-effects are denormalized onto `Part.allocatedQty` (soft) and `Part.reservedQty` (hard; `available = currentStock − reservedQty`); assembling a designator issues stock from the line's source location via the shared inventory primitive, and completion receives the output part. Split allocation across multiple matching parts remains out of scope.
