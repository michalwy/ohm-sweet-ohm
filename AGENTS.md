...
- Preserve part parameter values when category assignments change. Removing a local parameter attachment from a category or changing a part's primary category must not delete existing `PartParameterValue` records; if the parameter later becomes effective for that part again, reuse the saved value.
