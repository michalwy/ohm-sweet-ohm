...
- Never run `prisma migrate dev`, `prisma migrate reset`, or `prisma db push` automatically — these touch the user's development database. When a schema change is needed, generate only the migration SQL file with `pnpm exec prisma migrate dev --create-only --name <name>`. The migration will be applied the next time the user runs the stack. Do not attempt to fix migration checksum drift by resetting the database.
