...
**In CI the check runs with a declared blind spot.** `GITHUB_TOKEN` can read the ruleset but not `bypass_actors`, so the workflow passes `--allow-unverifiable=bypass_actors` — and only while no `RULESET_TOKEN` secret exists. That declaration is written in the workflow and reviewed like any other change, not inferred at runtime by the thing being excused. The run names it in its summary, so a green means *everything this caller was asked to check matches, and what it was asked to check is in the repository*.
