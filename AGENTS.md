...
**In CI the check runs with a declared blind spot.** `GITHUB_TOKEN` can read the ruleset but not `bypass_actors`, so the workflow passes `--allow-unverifiable=bypass_actors` — and only while no `RULESET_READ_TOKEN` secret exists. That declaration is written in the workflow and reviewed like any other change, not inferred at runtime by the thing being excused. The run names it in its summary, so a green means *everything this caller was asked to check matches, and what it was asked to check is in the repository*.

Three properties make that different from downgrading a failure to a warning, and none of them is optional. **Anything absent without a declaration still fails**, so the blind spot cannot grow — if the platform redacts a second field tomorrow, the job goes red. **The declaration does not apply to `--write`**, which still refuses outright, because not checking a field is a concession while recording an artifact without it manufactures an incomplete artifact that looks complete. And it **retires itself**: run by a caller who can see the field, the check says the declaration is unnecessary and compares normally.

The alternative — staying permanently red until someone creates a credential — was rejected for a reason worth keeping. A scheduled run's entire delivery channel is the mail GitHub sends on failure, so a job red for a known, permanent, one-action gap makes red *ambiguous*: real drift arrives in the same envelope as the known gap and cannot be told from it without opening the run. That kills the primary instrument to protect a secondary one.
