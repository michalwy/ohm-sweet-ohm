...
- **A task session is a separate session, not a subagent.** It gets its own worktree and outlives the turn that spawned it. Its prompt must carry the spawning lead's session id, because a session cannot infer who spawned it and otherwise has nobody to report to.
