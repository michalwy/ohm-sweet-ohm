...
The check finds the gate by **what it gates, not what it is called**: every ruleset whose target is a branch and whose conditions cover the default branch. What gates a branch is the union of every ruleset matching it, so selecting by name would make the artifact's completeness depend on a string — an organisation ruleset called anything else would change no field, produce no diff, and simply never be mentioned. **A second match is drift**, whatever it claims to target, because it can add requirements or bypass actors this repository never reviewed. Nothing can inherit here today only because the account owner is a `User` rather than an organisation; that is a fact about the account, not about the check, and it expires the day this repository moves.
...
**CI verifies the whole artifact, `bypass_actors` included.** A `RULESET_READ_TOKEN` secret with administration read was created on 2026-09-08; before it, `GITHUB_TOKEN` returned the ruleset in a reduced view without that field and the workflow declared the gap with `--allow-unverifiable=bypass_actors`. **That declaration is retired, and the branch that applied it is deleted rather than the flag alone** — with the branch gone, deleting the secret leaves the field absent and *undeclared*, which exits 1. Keeping the conditional would have let one deleted secret return the check silently to green-and-partial, which is the state the declaration mechanism exists to prevent.

`GH_TOKEN` still falls back to `github.token` on purpose: the run then reaches the comparison and fails with a message about coverage, rather than dying on an authentication error that says nothing. An **expired** token behaves differently again and is worth knowing apart — a non-empty secret that the API rejects, so the run fails at the fetch.

The script keeps `--allow-unverifiable`, with its enumerated allowlist of one field. It is the estate's shared shape and the mechanism a fresh project needs before it has a credential; nothing here passes it.
...
- Use `pnpm lint` for ESLint verification. Run it before finishing any task that touches source files. Fix all errors; warnings may be left as-is.
  - **Known divergence, as of 2026-09-07: `pnpm lint` reports 93 errors locally and zero in CI**, and CI is the one telling the truth about this repository's own source. Every one of those errors is in `site/dist/**` or `site/.astro/**` — build output that is untracked and gitignored, so CI, which never builds the site, cannot see it. `eslint.config.mjs` does not ignore those paths. So the rule above is unrunnable as written on any machine where the site has been built. Until the config is fixed, check that the errors you see are outside `site/`, and do not "fix" generated files. A verification whose result depends on where it runs is not a verification, so this is recorded as a defect awaiting a fix, not as a quirk to work around.
