...
- Use `pnpm test:e2e` for browser-level verification of interactive UI changes. Do not put Prisma-importing tests in `tests/e2e/` — those belong in `tests/integration/`. `pnpm test:e2e` always resets and re-seeds the e2e database before running Playwright — this is safe and expected because the e2e database (`ohm_sweet_ohm_e2e` on port 5433) is an isolated test instance. Run it as-is; do not attempt a manual seed or workaround.
...
- Before finishing any task that changes UI components (dialogs, forms, tables, navigation, or other interactive flows), run `pnpm test:e2e` and confirm all tests pass.
- When both server-side and UI files change in the same task, run both `pnpm test:integration` and `pnpm test:e2e`.
