...
- Use `pnpm lint` for ESLint verification. Run it before finishing any task that touches source files. Fix all errors; warnings may be left as-is.
  - Fixed 2026-09-08, and recorded because the shape recurs: `pnpm lint` used to report **93 errors locally and zero in CI**, all of them in `site/dist/**` and `site/.astro/**` — the Astro site's build output, gitignored, and never built by CI. So the rule above was unrunnable as written on any machine where the site had been built, and CI structurally could not reveal it. `eslint.config.mjs` now ignores both paths. The site's own sources are still linted. A verification whose result depends on where it runs is not a verification, so a divergence between your result and CI's is a defect in the check rather than something to work around.
