...
- Use the category-parameter model documented in `docs/decisions/0011-category-parameters.md`: parameters are workspace-scoped dictionary records, category attachments/overrides define defaults, sort order, and `isPrimary`, category `valueParameterId` controls the parts-list Value column, and all parameters are optional.
- For part forms, the effective parameter set is the primary category parameters plus secondary category parameters, deduplicated by parameter id. If a parameter appears in both categories, primary category configuration always wins. The parts-list Value column comes only from the primary category effective `valueParameterId`.
- Preserve part parameter values when category assignments change. Removing a local parameter attachment or changing a part's primary or secondary category must not delete existing `PartParameterValue` records; if the parameter later becomes effective for that part again, reuse the saved value.
- When a local category parameter attachment overrides an inherited attachment, detaching the local attachment should reveal the inherited effective parameter instead of removing that parameter from the category.
